...
export class NES {
  ...
  private init(gameFile: ArrayBuffer, keys: ControllerKeys): void {
    const mapper = this.createMapper(gameFile);
    this.screen = new CanvasScreen(
      document.getElementById('screen') as HTMLCanvasElement
    );
    this.ppu = new PPU(new PPUBus(mapper), this.screen);
    this.controller1 = new Controller();
    this.controller1.bindKeys(keys);
    this.controller2 = new Controller();
    this.cpu = new CPU(
      new CPUBus(this.ppu, mapper, this.controller1, this.controller2)
    );
  }
  ...
  private destroy(): void {
    ...
    this.cpu = this.ppu = this.screen = null!;
  ...
  private createMapper(buffer: ArrayBuffer): Mapper {
    ...
    const mapperNumber = ((arr[6] & 0xf0) >> 4) | (arr[7] & 0xf0);
    const prgStart = headerSize + trainerSize;
    const prgEnd = prgStart + prgSize;
    const prg = arr.slice(prgStart, prgEnd);
    const chr = arr.slice(prgEnd, prgEnd + chrSize);

    switch (mapperNumber) {
      case 0:
        return new Mapper0(prg, chr, mirroring);
